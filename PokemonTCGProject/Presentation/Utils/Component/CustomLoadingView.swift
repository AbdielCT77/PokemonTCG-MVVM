...
public class CustomLoadingView: UIView {
    public var dotParms: LoadingActivityParam?
    private var isAnimating = false
    private var rootViewController: UIWindow?
    private var info: UILabel = {
        let label = addComponent.label(
            id: "loading", text: "Please Wait", font: .systemFont(ofSize: 16),
            addColor: BaseColor.white, align: .center
        )
        return label
    }()
    ...
    func loadDotActivityIndicatorParms() -> LoadingActivityParam {
        let dotParms = LoadingActivityParam()
        ...
        dotParms.defaultColor = .navigationColor
    ...
    private func createCircle(with index: Int) {
        if let dotParms = dotParms {
            let origonX = CGFloat(
                (CGFloat(index) * (dotParms.activityViewWidth / CGFloat(dotParms.numberOfCircles) +
                                   dotParms.internalSpacing)
                )
            )
            ...
            let circle = UIView(
                frame: CGRect(
                    x: origonX, y: origonY, width: sizeW, height: sizeW
                )
            )
            ...
            circle.layer.add(
                createAnimation(
                    withDelay: (CGFloat(index) * dotParms.animationDelay)
                ), forKey: "scale"
            )
